...
namespace CodeJam.Ranges
{
    ...
    public static class Range
    {
        ...
        {
            return new Range<TValue>(start, end,
        ...
        public static Range<TValue> StartsWith<TValue>(TValue start, bool include = true)
            where TValue : IComparable<TValue>
        {
            return new Range<TValue>(start, default(TValue),
        ...
        public static Range<TValue> Simple<TValue>(TValue value)
            where TValue : IComparable<TValue>
        {
            return Create(value, value, true, true);
        }

        public static Range<TValue> EndsWith<TValue>(TValue end, bool include = true)
            where TValue : IComparable<TValue>
        {
            return new Range<TValue>(default(TValue), end,
                RangeOptions.HasEnd | (include ? RangeOptions.IncludingEnd : RangeOptions.None));
        }

        public static Range<TValue> Empty<TValue>()
            where TValue : IComparable<TValue>
        {
            return Range<TValue>.Empty;
        }

        public static Range<TValue> Full<TValue>()
            where TValue : IComparable<TValue>
        {
            return Range<TValue>.Full;
        }
    }
        ...
        where TValue : IComparable<TValue>
    {
        ...
        {
            get { return _options.HasFlag(RangeOptions.HasStart); }
        }

        public bool HasEnd
        {
            get { return _options.HasFlag(RangeOptions.HasEnd); }
        }

        public bool IncludeStart
        {
            get { return _options.HasFlag(RangeOptions.IncludingStart); }
        }

        public bool IncludeEnd
        {
            get { return _options.HasFlag(RangeOptions.IncludingEnd); }
        }

        public bool IsEmpty
        {
            get { return _options.HasFlag(RangeOptions.IsEmpty); }
        }

        public bool IsFull
        {
            get { return _options == RangeOptions.None; }
        }

        private RangeValue StartValue
        {
            get { return new RangeValue(Start, HasStart, IncludeStart); }
        }

        private RangeValue EndValue
        {
            get { return new RangeValue(End, HasEnd, IncludeEnd); }
        }
        ...
        {
            return new Range<TValue>(newStart, End,
        ...
        {
            return new Range<TValue>(Start, newEnd,
                (IncludeStart ? RangeOptions.IncludingStart : RangeOptions.None) |
        ...
        public Range<TValue> Union(Range<TValue> other)
        {
            ...
            newStart = RangeValue.MinValue(other.StartValue, current.StartValue, false);
            newEnd = RangeValue.MaxValue(other.EndValue, current.EndValue, false);
        ...
        public IEnumerable<Range<TValue>> Intersect(Range<TValue> other)
        {
            ...
            {
                if (other.HasStart)
                    newStart = RangeValue.MaxValue(current.StartValue, other.StartValue, true);
                else
                    newStart = current.StartValue;
            }
            else
            {
            ...
            {
                if (other.HasEnd)
                    newEnd = RangeValue.MinValue(current.EndValue, other.EndValue, true);
                else
                    newEnd = current.EndValue;
            }
            else
            {
        ...
        public string DisplayValue()
        {
			...
			if (HasStart && HasEnd && object.Equals(Start, End))
			{
